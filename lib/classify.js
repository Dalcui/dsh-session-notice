...
export function isJsonBody(status, contentType) {
    if (status === 413 || status === 418)
        return false;
...
export function isKeyErrorMessage(message) {
 ...
 * @param bodyText - 响应正文。
 */
export function classifyPush(status, contentType, bodyText) {
 ...
 */
export function classifyPing(status, contentType, bodyText) {
    if (!isJsonBody(status, contentType)) {
        if (status === 200) {
 ...
 */
export function classifyRegister(status, contentType, bodyText) {
    if (!isJsonBody(status, contentType)) {
