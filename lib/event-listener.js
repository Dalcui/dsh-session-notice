 ...
 */
import { composePushPayload, MAX_REQUEST_BYTES } from './bark-service.js';
 ...
 */
export function createTurnEndHandler(deps) {
    const dedup = createDedupLedger();
    return (session, event) => {
        if (event.type !== 'turn/end')
            return;
        if (!deps.isEnabled(session.id))
            return;
        ...
        // 之后再开启时同一轮次的首次处理仍能推送。
        if (!dedup.test(`${session.id}:${event.seq ?? -1}`))
            return;
        ...
        void deps.deliver(payload).then((verdict) => {
            ...
            if (!verdict.ok)
                deps.logger?.warn(`[bark-notify] 推送未成功（${verdict.kind}）：${verdict.message}`);
        });
    };
}
