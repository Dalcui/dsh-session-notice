 ...
 */
export function truncateByBytes(text, maxBytes, suffix = '…') {
    if (byteLength(text) <= maxBytes)
        return { text, truncated: false };
    let budget = maxBytes - byteLength(suffix);
    if (budget < 0)
        return { text: '', truncated: true }; // 连省略号都放不下：返回空串，守住字节契约
    let accumulated = '';
    let accumulatedBytes = 0;
    for (const cp of text) {
        const cpBytes = byteLength(cp);
        if (accumulatedBytes + cpBytes > budget)
            break;
        accumulated += cp;
 ...
 */
export function bodyTail(totalChars, budget) {
    const tail = `…（共 ${totalChars} 字，见 DSH）`;
    const tailBytes = byteLength(tail);
    if (tailBytes >= budget)
        return { tail: '…', rest: Math.max(0, budget - 1) };
