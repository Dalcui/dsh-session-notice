...
function sliceChars(text: string, limit: number): string {
  if (limit <= 0) return ''
  const cps = [...text]
...
function pickRepresentative(blocks: string[]): string | undefined {
  ...
  const first = pool[0]
  if (first !== undefined && !isFenced(first)) return first
 ...
 * @returns 摘要文本与是否截断。
 */
export function summarizeBody(text: string, limitChars: number): Summary {
  const trimmed = text.trim()
  if (trimmed.length === 0) return { text: '', truncated: false }
  if (charLength(trimmed) <= limitChars) return { text: trimmed, truncated: false }
  ...
  if (budget <= 0) return { text: sliceChars(first, limitChars), truncated: true }
 ...
 */
export function decorateSummary(summary: Summary, totalChars: number, unknownText: string): string {
  if (summary.text.length === 0) return unknownText
