...
/** GET /state */
function handleState(deps: BarkRpcDeps, res: ServerResponse): void {
...
/** POST /toggle */
async function handleToggle(deps: BarkRpcDeps, req: IncomingMessage, res: ServerResponse): Promise<void> {
...
/** POST /test */
async function handleTest(deps: BarkRpcDeps, res: ServerResponse): Promise<void> {
  const settings = deps.getSettings()
...
function wrap(
  method: 'GET' | 'POST',
  run: (req: IncomingMessage, res: ServerResponse) => void | Promise<void>,
