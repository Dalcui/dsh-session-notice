...
- **异常停止**（出错 / 中止 / 被阻塞 / Token 上限 / 中断）→ 推送停止原因与必要信息
...
3. **开启通知**：回到会话，点头部的铃铛按钮（单色图标，跟随主题）在「会通知 / 不通知」间切换。开启后该会话每次轮次停止都会推送。
...
- **正文双层预算**：
  - *展示层* —— 默认 200 字摘要（方案 C：首段 + 末段，跳过代码块，按码点不劈 emoji），超出缀「（共 N 字）」。因为 iOS 横幅约 4 行（≈80–120 中文字），全量塞入既看不全又笨重。
...
npm test                         # node:test（80 用例：纯函数 + 有状态 mock + 假接线）
npm run typecheck
```
